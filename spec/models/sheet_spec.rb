  ...
  describe 'スキルマップ保存' do
    ...
    context 'スキルマップを登録できないとき' do
      it 'タイトルがないと登録できない' do
        @sheet_item.title = ''
        @sheet_item.valid?
        expect(@sheet_item.errors.full_messages).to include('タイトルを入力してください')
      end
      ...
      it 'タイトルは40文字以内でないと登録できない' do
        ...
        @sheet_item.valid?
        expect(@sheet_item.errors.full_messages).to include('タイトルは40文字以内で入力してください')
      end
      it 'テキストは100文字以内でないと登録できない' do
        @item.text = 'a' * 101
        @item.valid?
        expect(@item.errors.full_messages).to include('テキストは100文字以内で入力してください')
      end
    end
  end
end
